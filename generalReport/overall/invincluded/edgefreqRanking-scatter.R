...
for (key in ls(catdata)) {
	...
	if (i==1) {
		...
		plot (c(1:length(vdata)), sort(vdata), col=colors[i], log="xy",pch=pches[i],xlim=c(1,1000000), ylim=c(1,1000000), xlab="Call",ylab="Frequency")
		...
		points(sort(vdata), col=colors[i],pch=pches[i])
	}
	i <- i+1
}
